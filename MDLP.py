...
class MDLP_Discretizer(TransformerMixin):
    ...
    def MDLPC_criterion(self, X, y, cut_point):
        """
    ...
    def compute_boundary_points_all_features(self):
        ...
        boundaries[:, self._col_idx] = np.apply_along_axis(padded_cutpoints_array, 0, self._data_raw[:, self._col_idx],
    ...
    def generate_bin_descriptions(self):
        ...
        for attr in self._col_idx:
            if len(self._cuts[attr]) == 0:
                ...
                bin_labels = ['%s..%s' % (str(cuts[i]), str(cuts[i + 1])) for i in start_bin_indices]
    ...
    @property
    def bin_descriptions(self):
        return self._bin_descriptions
